...
\begin{itemize}
	...
	\begin{itemize}
		\item Costo: $\left(b_{r}+b_{s}\right)$
	\end{itemize}
	...
	El método de junta Hash versión GRACE se usa para calcular la junta $R \bowtie_{R.A=S.A}S$ cuando ninguna de las tablas entran en memoria. Lo que se hace es generar 2 conjuntos de $M$ particiones. Se utilizan dos funciones de hash: una para generar las buffers $h:A\to[0,M-1]$, y otra para calcular qué tuplas hacen \emph{join}.

	\subitem{Etapa 1: Particionamiento}
		\subsubitem{-} Por cada tupla $t$ de $R$ se aplica la función de hashing $h(t)$ para saber a qué buffer enviar la tupla, sí el buffer se completa se graba en disco. Al finalizar la etapa, se graban los $M$ buffers a disco.
...
\textit{De este modo R queda particionada en $R_{i}$ subconjuntos con $0 \leq i \leq M-1$, del mismo modo que $S$ queda particionada en $S_{i}$ subconjuntos.}
	...
	\subitem{Etapa 2: Junta} 
		\subsubitem{-} Se leen las $R_{i}$ particiones. A cada tupla leída se le aplica la función de hash $h_{2}$ para saber dónde guardarla en una tabla de hash en memoria ($TH$). 
		
		\subsubitem{-} Se calcula $h_{2}$ para cada tupla de $S_{i}$ y se verifica si en $TH$ existe alguna tupla juntable, si es así se emite una tupla de resultado y se continua con la siguiente tupla de $S_{i}$.
	\begin{itemize}
		\item Costo: $2\left(b_{s}+b_{r}\right)$
	\end{itemize}
